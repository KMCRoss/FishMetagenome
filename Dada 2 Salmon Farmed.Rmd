...
```{r}
path <- "C:/Users/Karen/Documents/ProjectDada/fastqfiles/Salmon/Salmon Farmed" 
# CHANGE ME to the directory containing the fastq files after unzipping.
list.files(path)
```
## Forward and reverse fastq filenames must have format: SAMPLENAME_R1_001.fastq and SAMPLENAME_R2_001.fastq
```{r}
...
# write.csv(k, "kSalF.csv", row.names = rownames(N))
G12 <- 1-k[1,2]
H12 <- 1-k[2,3]
GH <- mean(G12, H12)
...
rownames(D) <- c("try", "t1", "t2", "t3", "Rich", "G12", "H12", "GH", "NoChim", "M12", "BootClass", "BootGenus", "input", "filtered", "denoisedF", "denoisedR", "merged", "nonchim" )
M12 <- max(D[12,])
#saveRDS(D, "SalmonFarmedDada2.rds")
...
saveRDS(De, "DeSalF.rds")
```

## Export data to data frame and plot results
```{r}
Dc <- readRDS("DeSalF.rds")
De <- rbind(Dd, Dc)
...
##plot.default(De$BootGenus, De$GH, xlab = "Bootstrap values", ylab = "Duplication")
```{r}
write.csv(as.matrix(t(De)), "DeSalF.csv")
...
t2 <- DeFit$t2[1]
t3 <- DeFit$t3[1]
write.csv(as.matrix(t(DeFit)), "dadaFitSalF.csv")
