...
```{r}
path <- "C:/Users/Karen/Documents/ProjectDada/fastqfiles/Salmon/Salmon Wild" 
...
# write.csv(k, "kSalW.csv", row.names = rownames(N))
G12 <- 1-k[1,2]
H12 <- 1-k[2,3]
GH <- mean(G12, H12)
...
saveRDS(De, "DeSalW.rds")
```

## Export data to data frame and plot results
```{r}
Dc <- readRDS("DeSalW.rds")
De <- rbind(Dd, Dc)
...
##plot.default(De$BootGenus, De$GH, xlab = "Bootstrap values", ylab = "Duplication")
```{r}
write.csv(as.matrix(t(De)), "DeSalW.csv")
...
t2 <- DeFit$t2[1]
t3 <- DeFit$t3[1]
write.csv(as.matrix(t(DeFit)), "dadaFitSalW.csv")
