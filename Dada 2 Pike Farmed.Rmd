...
```{r}
path <- "C:/Users/Karen/Documents/ProjectDada/fastqfiles/Pike/Pike Farmed" 
...
# write.csv(k, "kPkF.csv", row.names = rownames(N))
G12 <- 1-k[1,2]
H12 <- 1-k[2,3]
GH <- mean(G12, H12)
...
##plot.default(De$BootGenus, De$GH, xlab = "Bootstrap values", ylab = "Duplication")
```{r}
write.csv(as.matrix(t(De)), "DePkF.csv")
...
t2 <- DeFit$t2[1]
t3 <- DeFit$t3[1]
write.csv(as.matrix(t(DeFit)), "dadaFitPkF.csv")
